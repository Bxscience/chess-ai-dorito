...
public class GameManager : MonoBehaviour
{
    ...
    void Start() {
        ...
        aiPlayer = (GameSettings.playAgainstAI == 1) ? null : ((GameSettings.playerColor == 0) ? GameManager.black : GameManager.white);
        ChessAI.searchDepth = GameSettings.aiDifficulty + 2;
    ...
    public void NextPlayer() {
        ...
        if (IsCheckmate(enumPieces, currentPlayer.color) && InCheck(enumPieces, otherPlayer.color)) {
            ...
            FindObjectOfType<GameOverUI>().DisplayGameOver($"Checkmate\n{(otherPlayer == white ? "White" : "Black")} Wins!");
        }
        else if (IsCheckmate(enumPieces, currentPlayer.color)) {
            Debug.Log("stalemate");
            Destroy(board.GetComponent<TileSelector>());
            Destroy(board.GetComponent<MoveSelector>());
            FindObjectOfType<GameOverUI>().DisplayGameOver("Draw");
        }
